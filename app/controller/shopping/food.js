  ...
  async updateFood() {
    ...
    try {
      ...
      if (new_category_id !== category_id) {
        ...
        targetFoodMenu.foods.push(food._id);
        await service.shopping.food.updateCategory(new_category_id, targetFoodMenu);
        const foodIndex = foodMenu.foods.findIndex(item => item === food._id);
        if (foodIndex !== -1) {
          foodMenu.foods.splice(foodIndex, 1);
          await service.shopping.food.updateCategory(category_id, foodMenu);
        }
